package net.casper.litmus.serder;

import com.casper.sdk.model.block.JsonBlockBody;
import com.casper.sdk.model.key.PublicKey;
import com.syntifi.crypto.key.encdec.Hex;
import org.junit.jupiter.api.Test;

import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;

/**
 * @author ian@meywood.com
 */
class JsonBlockBodyByteSerializerTest {

    @Test
    void toBytes() throws Exception {

        final byte[] proposerBytes = {
                (byte) 0x2,
                (byte) 0x3,
                (byte) 0x55,
                (byte) 0xc,
                (byte) 0x87,
                (byte) 0xf2,
                (byte) 0xe3,
                (byte) 0x6d,
                (byte) 0xed,
                (byte) 0xbf,
                (byte) 0x37,
                (byte) 0x4,
                (byte) 0x9e,
                (byte) 0x51,
                (byte) 0xfe,
                (byte) 0xf3,
                (byte) 0x71,
                (byte) 0x6b,
                (byte) 0x8c,
                (byte) 0xe2,
                (byte) 0xac,
                (byte) 0x48,
                (byte) 0x52,
                (byte) 0x9e,
                (byte) 0xa8,
                (byte) 0x52,
                (byte) 0xb6,
                (byte) 0x17,
                (byte) 0x99,
                (byte) 0xc3,
                (byte) 0xd9,
                (byte) 0x9e,
                (byte) 0xac,
                (byte) 0x32
        };

        final byte[][] deployHashes = {
                {
                        (byte) 0xd1,
                        (byte) 0x88,
                        (byte) 0xb5,
                        (byte) 0x27,
                        (byte) 0xe9,
                        (byte) 0x93,
                        (byte) 0xe0,
                        (byte) 0x7c,
                        (byte) 0x9c,
                        (byte) 0x1,
                        (byte) 0xd7,
                        (byte) 0x24,
                        (byte) 0x7,
                        (byte) 0x78,
                        (byte) 0x7b,
                        (byte) 0xe4,
                        (byte) 0x7d,
                        (byte) 0xa7,
                        (byte) 0x72,
                        (byte) 0x6e,
                        (byte) 0xaa,
                        (byte) 0x63,
                        (byte) 0x1e,
                        (byte) 0x96,
                        (byte) 0x1b,
                        (byte) 0x80,
                        (byte) 0x38,
                        (byte) 0x61,
                        (byte) 0xc1,
                        (byte) 0x9f,
                        (byte) 0xcd,
                        (byte) 0xe9
                },
                {
                        (byte) 0xd3,
                        (byte) 0xe1,
                        (byte) 0xcb,
                        (byte) 0x70,
                        (byte) 0x8e,
                        (byte) 0xb5,
                        (byte) 0x5b,
                        (byte) 0x96,
                        (byte) 0x1c,
                        (byte) 0x42,
                        (byte) 0xec,
                        (byte) 0x78,
                        (byte) 0xc2,
                        (byte) 0xf5,
                        (byte) 0x1c,
                        (byte) 0xb9,
                        (byte) 0xb5,
                        (byte) 0x3f,
                        (byte) 0xc9,
                        (byte) 0x6d,
                        (byte) 0x36,
                        (byte) 0xc2,
                        (byte) 0x14,
                        (byte) 0x96,
                        (byte) 0x67,
                        (byte) 0xfe,
                        (byte) 0x8,
                        (byte) 0x43,
                        (byte) 0xdb,
                        (byte) 0x48,
                        (byte) 0xa2,
                        (byte) 0x6a
                },
                {
                        (byte) 0x6a,
                        (byte) 0x5a,
                        (byte) 0x48,
                        (byte) 0x61,
                        (byte) 0x4f,
                        (byte) 0x72,
                        (byte) 0xa1,
                        (byte) 0x3c,
                        (byte) 0x19,
                        (byte) 0x58,
                        (byte) 0x6e,
                        (byte) 0xbc,
                        (byte) 0x5a,
                        (byte) 0x96,
                        (byte) 0x41,
                        (byte) 0xf8,
                        (byte) 0xf7,
                        (byte) 0x17,
                        (byte) 0xcd,
                        (byte) 0xfc,
                        (byte) 0x30,
                        (byte) 0xf8,
                        (byte) 0x15,
                        (byte) 0x8c,
                        (byte) 0xe,
                        (byte) 0xb4,
                        (byte) 0x26,
                        (byte) 0xc8,
                        (byte) 0x55,
                        (byte) 0x77,
                        (byte) 0x63,
                        (byte) 0xde
                }
        };

        final byte[][] transferhashes = {
                {
                        (byte) 0x10,
                        (byte) 0xe7,
                        (byte) 0x6b,
                        (byte) 0x42,
                        (byte) 0xd4,
                        (byte) 0xcd,
                        (byte) 0x6,
                        (byte) 0x57,
                        (byte) 0xee,
                        (byte) 0x6,
                        (byte) 0xc1,
                        (byte) 0xfb,
                        (byte) 0xf1,
                        (byte) 0x3e,
                        (byte) 0x86,
                        (byte) 0x75,
                        (byte) 0xa7,
                        (byte) 0xb1,
                        (byte) 0x94,
                        (byte) 0x2f,
                        (byte) 0x7f,
                        (byte) 0x23,
                        (byte) 0xc9,
                        (byte) 0xeb,
                        (byte) 0x6a,
                        (byte) 0x30,
                        (byte) 0xa4,
                        (byte) 0x47,
                        (byte) 0x50,
                        (byte) 0xd,
                        (byte) 0xc4,
                        (byte) 0x49
                },
                {
                        (byte) 0xea,
                        (byte) 0x53,
                        (byte) 0xd6,
                        (byte) 0x92,
                        (byte) 0x78,
                        (byte) 0xcb,
                        (byte) 0x6f,
                        (byte) 0x8c,
                        (byte) 0xb5,
                        (byte) 0x62,
                        (byte) 0xfe,
                        (byte) 0xbc,
                        (byte) 0x21,
                        (byte) 0xf,
                        (byte) 0xa0,
                        (byte) 0xfb,
                        (byte) 0x28,
                        (byte) 0xf5,
                        (byte) 0x10,
                        (byte) 0xf3,
                        (byte) 0xf9,
                        (byte) 0xea,
                        (byte) 0xb5,
                        (byte) 0x25,
                        (byte) 0xb9,
                        (byte) 0x85,
                        (byte) 0xa,
                        (byte) 0x46,
                        (byte) 0x14,
                        (byte) 0x36,
                        (byte) 0xd9,
                        (byte) 0x29
                },
                {
                        (byte) 0xa2,
                        (byte) 0x16,
                        (byte) 0x98,
                        (byte) 0x1,
                        (byte) 0x36,
                        (byte) 0x82,
                        (byte) 0x72,
                        (byte) 0xe,
                        (byte) 0x22,
                        (byte) 0x26,
                        (byte) 0x55,
                        (byte) 0xea,
                        (byte) 0x97,
                        (byte) 0x94,
                        (byte) 0x68,
                        (byte) 0x47,
                        (byte) 0x67,
                        (byte) 0x4e,
                        (byte) 0xc1,
                        (byte) 0x7e,
                        (byte) 0x2c,
                        (byte) 0x53,
                        (byte) 0x5a,
                        (byte) 0x13,
                        (byte) 0x8,
                        (byte) 0xb,
                        (byte) 0x95,
                        (byte) 0x0,
                        (byte) 0x77,
                        (byte) 0x28,
                        (byte) 0x47,
                        (byte) 0x85
                },
                {
                        (byte) 0x88,
                        (byte) 0xdd,
                        (byte) 0xe3,
                        (byte) 0x6b,
                        (byte) 0x51,
                        (byte) 0x20,
                        (byte) 0x6f,
                        (byte) 0xfc,
                        (byte) 0x8d,
                        (byte) 0x79,
                        (byte) 0x27,
                        (byte) 0x72,
                        (byte) 0xf3,
                        (byte) 0xbf,
                        (byte) 0xce,
                        (byte) 0xa3,
                        (byte) 0xeb,
                        (byte) 0x91,
                        (byte) 0xa6,
                        (byte) 0x45,
                        (byte) 0x6,
                        (byte) 0x2a,
                        (byte) 0x93,
                        (byte) 0x3,
                        (byte) 0xd2,
                        (byte) 0xb8,
                        (byte) 0x84,
                        (byte) 0x37,
                        (byte) 0x5,
                        (byte) 0x58,
                        (byte) 0x4c,
                        (byte) 0x8b,
                }
        };


        final byte[] expectedBodyBytes = {
                (byte) 0x2,
                (byte) 0x3,
                (byte) 0x55,
                (byte) 0xc,
                (byte) 0x87,
                (byte) 0xf2,
                (byte) 0xe3,
                (byte) 0x6d,
                (byte) 0xed,
                (byte) 0xbf,
                (byte) 0x37,
                (byte) 0x4,
                (byte) 0x9e,
                (byte) 0x51,
                (byte) 0xfe,
                (byte) 0xf3,
                (byte) 0x71,
                (byte) 0x6b,
                (byte) 0x8c,
                (byte) 0xe2,
                (byte) 0xac,
                (byte) 0x48,
                (byte) 0x52,
                (byte) 0x9e,
                (byte) 0xa8,
                (byte) 0x52,
                (byte) 0xb6,
                (byte) 0x17,
                (byte) 0x99,
                (byte) 0xc3,
                (byte) 0xd9,
                (byte) 0x9e,
                (byte) 0xac,
                (byte) 0x32,
                (byte) 0x3,
                (byte) 0x0,
                (byte) 0x0,
                (byte) 0x0,
                (byte) 0xd1,
                (byte) 0x88,
                (byte) 0xb5,
                (byte) 0x27,
                (byte) 0xe9,
                (byte) 0x93,
                (byte) 0xe0,
                (byte) 0x7c,
                (byte) 0x9c,
                (byte) 0x1,
                (byte) 0xd7,
                (byte) 0x24,
                (byte) 0x7,
                (byte) 0x78,
                (byte) 0x7b,
                (byte) 0xe4,
                (byte) 0x7d,
                (byte) 0xa7,
                (byte) 0x72,
                (byte) 0x6e,
                (byte) 0xaa,
                (byte) 0x63,
                (byte) 0x1e,
                (byte) 0x96,
                (byte) 0x1b,
                (byte) 0x80,
                (byte) 0x38,
                (byte) 0x61,
                (byte) 0xc1,
                (byte) 0x9f,
                (byte) 0xcd,
                (byte) 0xe9,
                (byte) 0xd3,
                (byte) 0xe1,
                (byte) 0xcb,
                (byte) 0x70,
                (byte) 0x8e,
                (byte) 0xb5,
                (byte) 0x5b,
                (byte) 0x96,
                (byte) 0x1c,
                (byte) 0x42,
                (byte) 0xec,
                (byte) 0x78,
                (byte) 0xc2,
                (byte) 0xf5,
                (byte) 0x1c,
                (byte) 0xb9,
                (byte) 0xb5,
                (byte) 0x3f,
                (byte) 0xc9,
                (byte) 0x6d,
                (byte) 0x36,
                (byte) 0xc2,
                (byte) 0x14,
                (byte) 0x96,
                (byte) 0x67,
                (byte) 0xfe,
                (byte) 0x8,
                (byte) 0x43,
                (byte) 0xdb,
                (byte) 0x48,
                (byte) 0xa2,
                (byte) 0x6a,
                (byte) 0x6a,
                (byte) 0x5a,
                (byte) 0x48,
                (byte) 0x61,
                (byte) 0x4f,
                (byte) 0x72,
                (byte) 0xa1,
                (byte) 0x3c,
                (byte) 0x19,
                (byte) 0x58,
                (byte) 0x6e,
                (byte) 0xbc,
                (byte) 0x5a,
                (byte) 0x96,
                (byte) 0x41,
                (byte) 0xf8,
                (byte) 0xf7,
                (byte) 0x17,
                (byte) 0xcd,
                (byte) 0xfc,
                (byte) 0x30,
                (byte) 0xf8,
                (byte) 0x15,
                (byte) 0x8c,
                (byte) 0xe,
                (byte) 0xb4,
                (byte) 0x26,
                (byte) 0xc8,
                (byte) 0x55,
                (byte) 0x77,
                (byte) 0x63,
                (byte) 0xde,
                (byte) 0x4,
                (byte) 0x0,
                (byte) 0x0,
                (byte) 0x0,
                (byte) 0x10,
                (byte) 0xe7,
                (byte) 0x6b,
                (byte) 0x42,
                (byte) 0xd4,
                (byte) 0xcd,
                (byte) 0x6,
                (byte) 0x57,
                (byte) 0xee,
                (byte) 0x6,
                (byte) 0xc1,
                (byte) 0xfb,
                (byte) 0xf1,
                (byte) 0x3e,
                (byte) 0x86,
                (byte) 0x75,
                (byte) 0xa7,
                (byte) 0xb1,
                (byte) 0x94,
                (byte) 0x2f,
                (byte) 0x7f,
                (byte) 0x23,
                (byte) 0xc9,
                (byte) 0xeb,
                (byte) 0x6a,
                (byte) 0x30,
                (byte) 0xa4,
                (byte) 0x47,
                (byte) 0x50,
                (byte) 0xd,
                (byte) 0xc4,
                (byte) 0x49,
                (byte) 0xea,
                (byte) 0x53,
                (byte) 0xd6,
                (byte) 0x92,
                (byte) 0x78,
                (byte) 0xcb,
                (byte) 0x6f,
                (byte) 0x8c,
                (byte) 0xb5,
                (byte) 0x62,
                (byte) 0xfe,
                (byte) 0xbc,
                (byte) 0x21,
                (byte) 0xf,
                (byte) 0xa0,
                (byte) 0xfb,
                (byte) 0x28,
                (byte) 0xf5,
                (byte) 0x10,
                (byte) 0xf3,
                (byte) 0xf9,
                (byte) 0xea,
                (byte) 0xb5,
                (byte) 0x25,
                (byte) 0xb9,
                (byte) 0x85,
                (byte) 0xa,
                (byte) 0x46,
                (byte) 0x14,
                (byte) 0x36,
                (byte) 0xd9,
                (byte) 0x29,
                (byte) 0xa2,
                (byte) 0x16,
                (byte) 0x98,
                (byte) 0x1,
                (byte) 0x36,
                (byte) 0x82,
                (byte) 0x72,
                (byte) 0xe,
                (byte) 0x22,
                (byte) 0x26,
                (byte) 0x55,
                (byte) 0xea,
                (byte) 0x97,
                (byte) 0x94,
                (byte) 0x68,
                (byte) 0x47,
                (byte) 0x67,
                (byte) 0x4e,
                (byte) 0xc1,
                (byte) 0x7e,
                (byte) 0x2c,
                (byte) 0x53,
                (byte) 0x5a,
                (byte) 0x13,
                (byte) 0x8,
                (byte) 0xb,
                (byte) 0x95,
                (byte) 0x0,
                (byte) 0x77,
                (byte) 0x28,
                (byte) 0x47,
                (byte) 0x85,
                (byte) 0x88,
                (byte) 0xdd,
                (byte) 0xe3,
                (byte) 0x6b,
                (byte) 0x51,
                (byte) 0x20,
                (byte) 0x6f,
                (byte) 0xfc,
                (byte) 0x8d,
                (byte) 0x79,
                (byte) 0x27,
                (byte) 0x72,
                (byte) 0xf3,
                (byte) 0xbf,
                (byte) 0xce,
                (byte) 0xa3,
                (byte) 0xeb,
                (byte) 0x91,
                (byte) 0xa6,
                (byte) 0x45,
                (byte) 0x6,
                (byte) 0x2a,
                (byte) 0x93,
                (byte) 0x3,
                (byte) 0xd2,
                (byte) 0xb8,
                (byte) 0x84,
                (byte) 0x37,
                (byte) 0x5,
                (byte) 0x58,
                (byte) 0x4c,
                (byte) 0x8b,
        };

        var proposer = PublicKey.fromBytes(proposerBytes);

        var build = JsonBlockBody
                .builder()
                .proposer(proposer)
                .deployHashes(
                        List.of(
                                Hex.encode(deployHashes[0]),
                                Hex.encode(deployHashes[1]),
                                Hex.encode(deployHashes[2])
                        )
                )
                .transferHashes(
                        List.of(
                                Hex.encode(transferhashes[0]),
                                Hex.encode(transferhashes[1]),
                                Hex.encode(transferhashes[2]),
                                Hex.encode(transferhashes[3])
                        )
                )
                .build();

        var jsonBlockBodyByteSerializer = new JsonBlockBodyByteSerializer();
        var bytes = jsonBlockBodyByteSerializer.toBytes(build);

        assertThat(bytes).isNotNull();
        assertThat(bytes).isEqualTo(expectedBodyBytes);
    }
}
